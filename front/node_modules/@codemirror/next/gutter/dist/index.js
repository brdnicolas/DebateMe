...
class GutterMarker extends rangeset.RangeValue {
    ...
    at(pos) { return new rangeset.Range(pos, pos, this); }
}
GutterMarker.prototype.elementClass = "";
GutterMarker.prototype.startMapMode = GutterMarker.prototype.endMapMode = state.MapMode.TrackBefore;
const defaults = {
    style: "",
    renderEmptyElements: false,
    elementStyle: "",
    initialMarkers: () => rangeset.RangeSet.empty,
...
function gutter(config) {
    return [gutters(), activeGutters.of(state.fillConfig(config, defaults))];
}
...
const gutterView = view.ViewPlugin.fromClass(class {
    constructor(view$1) {
        ...
        this.dom.className = view.themeClass("gutters");
        this.dom.setAttribute("aria-hidden", "true");
        this.gutters = view$1.state.facet(activeGutters).map(conf => new SingleGutterView(view$1, conf));
        for (let gutter of this.gutters)
            this.dom.appendChild(gutter.dom);
        this.fixed = !view$1.state.facet(unfixGutters);
        if (this.fixed) {
        ...
        view$1.scrollDOM.insertBefore(this.dom, view$1.contentDOM);
    }
    update(update) {
        ...
        this.view.viewportLines(line => {
            ...
            if (Array.isArray(line.type))
                text = line.type.find(b => b.type == view.BlockType.Text);
            else
...
}).provide(view.PluginField.scrollMargins, value => {
...
class SingleGutterView {
    constructor(view$1, config) {
        this.view = view$1;
        this.config = config;
        this.elements = [];
        this.spacer = null;
        this.dom = document.createElement("div");
        this.dom.className = view.themeClass("gutter" + (this.config.style ? "." + this.config.style : ""));
        this.elementClass = view.themeClass("gutterElement" + (this.config.style ? "." + this.config.style : ""));
        for (let prop in config.domEventHandlers) {
            this.dom.addEventListener(prop, (event) => {
                let line = view$1.lineAtHeight(event.clientY);
                if (config.domEventHandlers[prop](view$1, line, event))
                    event.preventDefault();
            });
        }
        this.markers = config.initialMarkers(view$1);
        if (config.initialSpacer) {
            this.spacer = new GutterElement(view$1, 0, 0, [config.initialSpacer(view$1)], this.elementClass);
...
const lineNumberMarkers = state.Annotation.define();
