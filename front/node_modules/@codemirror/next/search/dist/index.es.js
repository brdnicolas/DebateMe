import { Decoration, themeClass, ViewPlugin, EditorView } from '../../view';
import { Annotation, StateField, Facet, EditorSelection, SelectionRange } from '../../state';
import { showPanel, panels } from '../../panel';
import { NormalizedKeymap, keymap } from '../../keymap';
import { isExtendingChar, isWordChar } from '../../text';
...
const panelKeymap = Facet.define({
    combine(keymaps) {
...
const replaceNext = view => {
    ...
    if (next)
        tr.setSelection(EditorSelection.single(next.from, next.to)).scrollIntoView();
    view.dispatch(tr);
    if (next)
        maybeAnnounceMatch(view);
    return true;
...
export { SearchCursor, closeSearchPanel, defaultSearchKeymap, findNext, findPrevious, openSearchPanel, replaceAll, replaceNext, search, selectMatches };
