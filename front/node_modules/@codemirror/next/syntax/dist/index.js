...
function syntaxIndentation(syntax) {
    return state.EditorState.indentation.of((state, pos) => {
...
function computeIndentation(state, ast, pos) {
    ...
    for (let scan = tree, scanPos = pos;;) {
        ...
        if (!last)
            break;
        if (last.type.prop(lezerTree.NodeProp.error) && last.start == last.end) {
            tree = scan;
            scanPos = last.start;
        }
        else {
...
function indentStrategy(type) {
    ...
    if (!strategy) {
        let delim = type.prop(lezerTree.NodeProp.delim);
        if (delim)
...
function bracketedAligned(context) {
    ...
    for (let pos = openToken.end;;) {
        ...
        if (!next)
            return null;
        if (!next.type.prop(lezerTree.NodeProp.skipped))
...
function syntaxFolding(syntax) {
    return state.EditorState.foldable.of((state, start, end) => {
...
class LezerSyntax {
    ...
    constructor(parser) {
        this.parser = parser;
        let setSyntax = state.Annotation.define();
        this.field = state.StateField.define({
            create(state) { return SyntaxState.advance(lezerTree.Tree.empty, parser, state.doc); },
            update(value, tr) { return value.apply(tr, parser, setSyntax); }
        });
        this.extension = [
            state.EditorState.syntax.of(this),
            this.field,
            view.ViewPlugin.define(view => new HighlightWorker(view, this, setSyntax)),
    ...
    languageDataAt(state$1, pos) {
        let type = this.parser.group.types[1];
        if (this.parser.hasNested) {
            let tree = this.getTree(state$1);
            let target = tree.resolve(pos);
            while (target) {
                if (target.type.prop(lezerTree.NodeProp.top)) {
                    type = target.type;
                    break;
                }
        ...
        return (type.prop(state.languageData) || nothing);
    }
}
const nothing = {};
class DocStream {
...
function mkMatchProp() { return new lezerTree.NodeProp({ deserialize(str) { return str.split(" "); } }); }
...
exports.closeNodeProp = closeNodeProp;
...
exports.openNodeProp = openNodeProp;
