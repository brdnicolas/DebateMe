...
class TagSystem {
    ...
    constructor(options) {
        ...
        this.prop = new lezerTree.NodeProp();
    ...
    /// to tag names.
    add(tags) {
        let match = lezerTree.NodeType.match(tags);
    ...
    highlighter(spec) {
        ...
        return [
            view.ViewPlugin.define(view => new Highlighter(view, this.prop, styling)).decorations(),
            view.EditorView.styleModule.of(styling.module)
        ];
    }
    /// @internal
    specificity(tag) {
...
class Styling {
    constructor(tags, spec) {
        ...
        this.module = new styleMod.StyleModule(modSpec);
    }
    match(tag) {
...
class Highlighter {
    ...
    update(update) {
        let syntax = update.state.facet(state.EditorState.syntax);
        if (!syntax.length) {
    ...
    buildDeco(ranges, tree) {
        ...
        function flush(pos, style) {
            if (pos > start && style)
                builder.add(start, pos, view.Decoration.mark({ class: style })); // FIXME cache these
            start = pos;
        }
