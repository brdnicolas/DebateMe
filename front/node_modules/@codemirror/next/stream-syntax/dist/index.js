...
class StreamSyntax {
    ...
    constructor(parser) {
        ...
        this.field = state.StateField.define({
            create(state) {
                let start = new SyntaxState(lezerTree.Tree.empty, [parserInst.startState(state)], 1, 0, null);
        ...
        this.extension = [
            ...
            view.ViewPlugin.define(view => new HighlightWorker(view, this.parser, this.field, setSyntax)),
            this.field,
            state.EditorState.indentation.of((state, pos) => {
    ...
    languageDataAt() {
        return (typeArray[this.parser.docType].prop(state.languageData) || {});
    }
}
...
class SyntaxState {
    ...
    advanceFrontier(parser, editorState, timeout, upto = editorState.doc.length) {
        ...
        let tree = lezerTree.Tree.build({ buffer,
            group: nodeGroup,
...
const nodeGroup = new lezerTree.NodeGroup(typeArray);
const warned = [];
function tokenID(tag) {
    ...
    if (id == null) {
        let tagID = 0;
        try {
            tagID = highlight.defaultTags.get(tag);
        }
        catch (e) {
        ...
        typeArray.push(new lezerTree.NodeType(tag ? tag.replace(/ /g, "_") : "_", highlight.defaultTags.prop.set({}, tagID), id));
    }
    return id;
}
function docID(props) {
    ...
    typeArray.push(new lezerTree.NodeType("document", obj, id));
    return id;
}
