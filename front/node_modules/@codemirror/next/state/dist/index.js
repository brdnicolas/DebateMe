...
class SelectionRange {
    ...
    static groupAt(state, pos, bias = 1) {
        ...
        if (linePos == 0)
            ...
            bias = -1;
        let read = linePos + (bias < 0 ? -1 : 0), type = text.charType(line.slice(read, read + 1));
        let from = pos, to = pos;
        for (let lineFrom = linePos; lineFrom > 0 && text.charType(line.slice(lineFrom - 1, lineFrom)) == type; lineFrom--)
            from--;
        for (let lineTo = linePos; lineTo < line.length && text.charType(line.slice(lineTo, lineTo + 1)) == type; lineTo++)
            to++;
...
const languageData = new lezerTree.NodeProp();
...
class ChangeDesc {
    ...
    /// @internal
    mapPos(pos, bias = -1, mode = exports.MapMode.Simple) {
        ...
        if (pos == to || pos == from) {
            if (from < pos && mode == exports.MapMode.TrackBefore || to > pos && mode == exports.MapMode.TrackAfter)
                return -pos - 1;
            return (from == to ? bias <= 0 : pos == from) ? from : from + length;
        }
        pos = from + (bias <= 0 ? 0 : length);
        return mode != exports.MapMode.Simple ? -pos - 1 : pos;
    }
...
class ChangeSet {
    ...
    mapInner(pos, bias, mode, fromI, toI) {
        ...
        for (let i = fromI - (dir < 0 ? 1 : 0), endI = toI - (dir < 0 ? 1 : 0); i != endI; i += dir) {
            ...
            if (pos > from && pos < to) {
                if (mode != exports.MapMode.Simple)
                    deleted = true;
...
class EditorState {
    ...
    get tree() {
        ...
        return syntax.length ? syntax[0].getTree(this) : lezerTree.Tree.empty;
    }
}
