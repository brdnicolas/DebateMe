...
/// diagnostics.
const setDiagnostics = state.Annotation.define();
const setState = state.Annotation.define();
const lintState = state.StateField.define({
    ...
    update(value, tr) {
        ...
        if (setDiag) {
            let ranges = view.Decoration.set(setDiag.map(d => {
                return d.from < d.to
                    ? view.Decoration.mark({
                        attributes: { class: view.themeClass("lintRange." + d.severity) },
                        diagnostic: d
                    }).range(d.from, d.to)
...
}).provideN(panel.showPanel, s => s.panel ? [s.panel] : [])
    .provide(view.EditorView.decorations, s => s.diagnostics);
const activeMark = view.Decoration.mark({ class: view.themeClass("lintRange.active") });
...
const openLintPanel = (view) => {
    ...
    if (view.state.field(lintState).panel)
        view.dom.querySelector(".cm-panel-lint ul").focus();
    return true;
};
...
function linter(source) {
    const runLintPlugin = view.ViewPlugin.fromClass(class {
        constructor(view) {
            this.view = view;
...
function renderDiagnostic(view$1, diagnostic) {
    let dom = document.createElement("li");
    dom.textContent = diagnostic.message;
    dom.className = view.themeClass("diagnostic." + diagnostic.severity);
    if (diagnostic.actions)
        for (let action of diagnostic.actions) {
            let button = dom.appendChild(document.createElement("button"));
            button.className = view.themeClass("diagnosticAction");
            button.textContent = action.name;
            button.onclick = button.onmousedown = e => {
                e.preventDefault();
                let found = findDiagnostic(view$1.state.field(lintState).diagnostics, diagnostic);
                if (found)
                    action.apply(view$1, found.from, found.to);
            };
        }
    // FIXME render source?
    return dom;
...
class DiagnosticWidget extends view.WidgetType {
    toDOM() {
        ...
        elt.className = view.themeClass("lintPoint." + this.value.severity);
        return elt;
    }
}
class PanelItem {
...
class LintPanel {
    ...
    moveSelection(selectedIndex) {
        ...
        this.view.dispatch(this.view.state.t()
            .setSelection(state.EditorSelection.single(selection.from, selection.to))
            .scrollIntoView()
