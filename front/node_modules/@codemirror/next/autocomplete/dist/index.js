...
function completeFromSyntax(state$1, pos) {
    let syntax = state$1.facet(state.EditorState.syntax);
    if (syntax.length == 0)
        return { items: [] };
    let { completeAt } = syntax[0].languageDataAt(state$1, pos);
    return completeAt ? completeAt(state$1, pos) : { items: [] };
}
...
const autocompleteConfig = state.Facet.define({
    combine(configs) {
...
function autocomplete(config = {}) {
    return [
        ...
        autocompletePlugin,
        state.Precedence.Fallback.set(style),
        tooltip.tooltips(),
        state.Precedence.Override.set(keymap.keymap({
...
function applyCompletion(view, option) {
    ...
    if (typeof apply == "string") {
        view.dispatch(view.state.t().replace(option.start, option.end, apply)
            .setSelection(state.EditorSelection.single(option.start + apply.length)));
    }
    else {
        apply(view);
    }
...
const setActiveCompletion = state.Annotation.define();
const activeCompletion = state.StateField.define({
    create() { return null; },
    update(prev, tr) {
        let set = tr.annotation(setActiveCompletion);
        if (set !== undefined)
            return set;
        return tr.annotation(state.Transaction.userEvent) == "input" ? "pending"
            : tr.docChanged || tr.selectionSet ? null
                : prev;
    }
}).provideN(tooltip.showTooltip, active => active instanceof ActiveCompletion ? [active.tooltip] : []);
...
const autocompletePlugin = view.ViewPlugin.fromClass(class {
    constructor(view) {
        this.view = view;
        this.stateVersion = 0;
        this.debounce = -1;
...
const style = view.EditorView.theme({
